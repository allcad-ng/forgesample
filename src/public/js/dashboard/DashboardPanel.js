...
class DashboardPanelChart extends DashboardPanel {
    load(parentDivId, divId, viewer, modelData) {
        if (this.propertyToUse !== 'progress' && !modelData.hasProperty(this.propertyToUse)){
...
class ModelData {
    constructor(viewer, progressData) {
        this._modelData = {};
        this._viewer = viewer;
    ...
    init(callback) {
        var _this = this;
        let completedIds = _this._progressData.AddCompleted.map(a => a.UniqueId);
        let activeIds = _this._progressData.AddActive.map(a => a.UniqueId);
        let plannedIds = _this._progressData.AddPlanned.map(a => a.UniqueId);

        _this._modelData['progress'] = [];
        _this._modelData['progress']['completed']= [];
        _this._modelData['progress']['planned']= [];
        _this._modelData['progress']['active']= [];
        ...
        _this.getAllLeafComponents(function (dbIds) {
            ...
            dbIds.forEach(function (dbId) {
                viewer.getProperties(dbId, function (props) {
                    ...
                    if (completedIds.includes(props.externalId)) {
                        _this._modelData['progress']['completed'].push(dbId);
                    }
                    else if (activeIds.includes(props.externalId)) {
                        _this._modelData['progress']['active'].push(dbId);
                    }
                    else if (plannedIds.includes(props.externalId))
                    {
                        _this._modelData['progress']['planned'].push(dbId);
                    }
